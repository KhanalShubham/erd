...
type TutorTab = 'chat' | 'socratic' | 'misconceptions' | 'mastery';
...
export const AiTutorModal: React.FC<AiTutorModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4 animate-in fade-in duration-150">
      <div className="bg-white border-2 border-zinc-900 rounded-lg w-full max-w-2xl h-[620px] max-h-[90vh] shadow-[4px_4px_0px_#18181B] flex flex-col overflow-hidden text-zinc-900 font-sans">
        {/* Notebook Header Bar */}
        ...
        {/* Tab Navigation */}
        <div className="flex items-center border-b border-zinc-200 bg-zinc-100 px-3 pt-1.5 gap-1 shrink-0 font-mono text-xs font-bold">
          <button
            onClick={() => setActiveTab('chat')}
            className={`px-3 py-1.5 rounded-t transition-colors border-t border-x ${
              activeTab === 'chat'
        ...
        <div className="flex-1 overflow-y-auto p-4 bg-[#FCFBF7]">
            ...
            <div className="flex flex-col h-full space-y-3">
              ...
              >
                <input
                  ...
                  placeholder="Ask a question (e.g., 'Why does FK go on the many side?')..."
                ...
                <div className="bg-white border border-zinc-300 rounded p-4 shadow-sm space-y-3">
                  ...
                  {socraticAnswerFeedback && (
                    ...
                    >
                      ...
                      <div>{socraticAnswerFeedback.feedback}</div>
                      <div className="text-[11px] text-zinc-700 italic">
                        {socraticAnswerFeedback.explanation}
                      </div>
              ...
              {socraticStepIdx === SOCRATIC_LMS_STEPS.length - 1 && socraticAnswerFeedback?.isOptimal && (
                <div className="p-3 bg-emerald-100 border border-emerald-400 rounded text-center space-y-2">
                  <span className="font-bold font-mono text-xs text-emerald-950 block">
                    🎉 Excellent! You have derived why the LMS needs an Associative Entity (ENROLLMENT)!
                  </span>
                  <button
                    onClick={() => {
          ...
          {/* TAB 3: Common Misconceptions Database */}
          {activeTab === 'misconceptions' && (
                      ...
                      {isExpanded && (
                        <div className="p-3 border-t border-zinc-200 bg-yellow-50/40 space-y-2 text-xs">
                          <div>
                            ...
                            <p className="text-zinc-700">{item.whyItsFalse}</p>
                          </div>
                          ...
                          <div>
                            ...
                            <p className="text-zinc-900 font-medium">{item.correctPrinciple}</p>
                          </div>
          ...
          {/* TAB 4: Student Concept Mastery Profile */}
          {activeTab === 'mastery' && (
            <div className="space-y-4 max-w-lg mx-auto">
              <div className="bg-white border border-zinc-300 rounded p-4 shadow-sm space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { name: 'Associative Entities (Junction Tables)', level: '90% (Mastered)', status: 'done' },
                    { name: 'Normalization (1NF, 2NF, 3NF)', level: '50% (Exploring)', status: 'learning' },
                  ].map((concept, idx) => (
              ...
              <div className="p-3 bg-yellow-50 border border-zinc-900 rounded shadow-[2px_2px_0px_#18181B] space-y-1">
                ...
                <p className="text-zinc-800 text-xs">
                  Review the Foreign Key explanation in the <strong>ENROLLMENT</strong> table and test the <strong>"🧪 Try Inserting Orphan Record"</strong> experiment to solidify referential integrity.
                </p>
              </div>
            </div>
          )}
